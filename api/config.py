...
class Settings:
    ...
    ai_solution_enabled: bool = _bool_env("AI_SOLUTION_ENABLED", True)
    ai_solution_cache_minutes: int = int(os.getenv("AI_SOLUTION_CACHE_MINUTES", "240"))
    ai_solution_max_per_user_day: int = int(os.getenv("AI_SOLUTION_MAX_PER_USER_DAY", "30"))
    ai_solution_max_per_customer_day: int = int(os.getenv("AI_SOLUTION_MAX_PER_CUSTOMER_DAY", "100"))
    ai_solution_model: str = os.getenv("AI_SOLUTION_MODEL", os.getenv("OPENAI_MODEL", "gpt-4o-mini"))
    anomaly_reopen_window_hours: int = int(os.getenv("ANOMALY_REOPEN_WINDOW_HOURS", "72"))
    anomaly_recurrence_window_hours: int = int(os.getenv("ANOMALY_RECURRENCE_WINDOW_HOURS", "24"))
    ...
    retention_cleanup_enabled: bool = _bool_env("RETENTION_CLEANUP_ENABLED", True)
    retention_cleanup_dry_run: bool = _bool_env("RETENTION_CLEANUP_DRY_RUN", False)
    retention_cleanup_interval_hours: int = int(os.getenv("RETENTION_CLEANUP_INTERVAL_HOURS", "24"))
    retention_telemetry_days: int = int(os.getenv("RETENTION_TELEMETRY_DAYS", "60"))
    retention_alarm_days: int = int(os.getenv("RETENTION_ALARM_DAYS", "180"))
    retention_insight_days: int = int(os.getenv("RETENTION_INSIGHT_DAYS", "365"))
    retention_communication_days: int = int(os.getenv("RETENTION_COMMUNICATION_DAYS", "365"))
    retention_collector_run_days: int = int(os.getenv("RETENTION_COLLECTOR_RUN_DAYS", "90"))
    retention_resolved_anomaly_days: int = int(os.getenv("RETENTION_RESOLVED_ANOMALY_DAYS", "365"))
    retention_batch_limit: int = int(os.getenv("RETENTION_BATCH_LIMIT", "10000"))
