...
O Eletrofrio JA é um cockpit operacional para refrigeração comercial. Ele coleta dados reais dos endpoints oficiais da Eletrofrio, organiza unidades, equipamentos, alarmes e telemetria no Supabase, aplica regras de criticidade e transforma esse volume técnico em prioridades claras para a operação.

O objetivo não é criar um chatbot genérico. O objetivo é reduzir o tempo entre identificar uma ocorrência e orientar a equipe. O painel mostra a visão geral da rede, os ativos monitorados, as ocorrências priorizadas e a rotina de coleta. O WhatsApp entra como canal operacional: a equipe recebe alertas relevantes e também pode perguntar, por exemplo, como está uma loja, quais equipamentos estão offline ou qual item exige atenção imediata.

A camada de diagnóstico usa RAG operacional: busca evidências reais no banco e sintetiza uma resposta curta. Se faltar dado, ela informa. Se a OpenAI falhar, o fallback local por regras mantém o sistema funcionando. Assim, a solução continua apresentável e útil mesmo em cenários de instabilidade.
...
A tela de Rotina Operacional usa o snapshot consolidado do Supabase como fonte principal para apresentação:

- lojas monitoradas;
- alarmes disponíveis;
- telemetria processada;
- status do WhatsApp;
- últimas execuções úteis;
- fila de ocorrências para envio ao WhatsApp.

Tentativas automáticas sem dados, como timeouts pontuais do endpoint oficial de unidades, são filtradas da experiência principal para manter o painel limpo. Durante apresentação, recomenda-se manter a automação desligada e executar coletas manuais quando necessário:
...
- `api/rules/*`: motor de regras operacionais auditáveis.
...
- `sql/003_operational_rules.sql`: regras operacionais e avaliações.

## Motor De Regras Operacionais

A plataforma não depende da IA para decidir prioridade. O backend cruza alarmes, telemetria, histórico e regras técnicas editáveis para gerar uma explicação auditável:
...
Para ativar em um Supabase novo ou antigo, execute no SQL Editor:

```text
sql/003_operational_rules.sql
```

Depois aplique regras sugeridas e reavalie:
